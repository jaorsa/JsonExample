...
/*
let sharedEtapaInstance = EtapaController()

class EtapaController: NSObject{
    var session = URLSession.shared
    var dataStr = ""
    
    class var Etapainstance: JsonClient{
        return sharedInstance
    }
    
    public func getRequest(url: String, completionHandler: @escaping(String,[Etapa], Error?) -> Void ){
        var model = [Etapa]()
        ...
        let task = session.dataTask(with: request){ (data, resp, error ) in
            ...
            do{
                ...
                model = try decoder.decode([Etapa].self, from: dataResponse)
