...
/*
let sharedEspecieInstance = EspecieController()

class EspecieController: NSObject{
    var session = URLSession.shared
    var dataStr = ""
    
    class var Especieinstance: JsonClient{
        return sharedInstance
    }
    
    public func getRequest(url: String, completionHandler: @escaping(String,[Especie], Error?) -> Void ){
        var model = [Especie]()
        ...
        let task = session.dataTask(with: request){ (data, resp, error ) in
            ...
            do{
                ...
                model = try decoder.decode([Especie].self, from: dataResponse)
